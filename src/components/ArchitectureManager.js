...
import { Plus, Trash2, Edit2, Check, X, ChevronDown, MoreHorizontal, Cloud, HardDrive } from "lucide-react";
...
const ArchitectureManager = ({ architectures, currentArchitectureId, setCurrentArchitectureId, createNewArchitecture, deleteArchitecture, renameArchitecture, isAuthenticated }) => {
	...
	const [showArchDetails, setShowArchDetails] = useState(false); // Уже false, оставляем как есть
		...
		<div className="mb-6">
			<div className="flex items-center gap-2 mb-4">
				...
				<button
					...
					title={showArchDetails ? "Скрыть детали" : "Показать детали"}
				>
					<h2 className="text-base font-bold text-gray-800 hover:text-blue-600 transition-colors">{isAuthenticated ? "Мои архитектуры" : "Локальные архитектуры"}</h2>
					<ChevronDown size={16} className={`transform transition-transform text-gray-500 ${showArchDetails ? "rotate-180" : ""}`} />
				</button>
			</div>
