...
const GameArchitectureDesigner = () => {
	...
	const classTypeColors = {
		...
		// Динамические цвета для пользовательских категорий
	...
	const getDynamicClassColor = (type) => {
		...
		// Генерируем цвет на основе хеша названия категории
		let hash = 0;
	...
	const removeCategory = (category) => {
		if (classCategories.length <= 1) return; // Оставляем хотя бы одну категорию

		const updatedCategories = classCategories.filter((c) => c !== category);
		// Если есть классы с удаляемой категорией, меняем их на первую доступную
	...
	React.useEffect(() => {
		const handleGlobalMouseMove = (e) => {
			if (isPanning) {
				...
				// Обновляем локальную камеру для плавности
				const newCamera = {
					...localCamera,
		...
		const handleGlobalMouseUp = () => {
			// При окончании панорамирования сохраняем в архитектуру
			if (isPanning) {
	...
	React.useEffect(() => {
		// Инициализация первой категории в форме
	...
	const handleCanvasMouseDown = (e) => {
		// Проверяем, что клик не по классу
			...
			<div className="w-80 bg-white border-r border-gray-200 p-4 overflow-y-auto">
					...
					{showArchitectureManager && (
						<div className="space-y-2">
							<div className="flex gap-1">
								...
								/>
								<button onClick={createNewArchitecture} className="bg-blue-500 text-white px-2 py-1 rounded text-xs hover:bg-blue-600">
									<Plus size={12} />
								</button>
							</div>
				...
				<div className="mb-6">
						...
						{classCategories.map((category) => (
							<div key={category} className="flex items-center justify-between p-2 bg-gray-50 rounded">
								...
								{classCategories.length > 1 && (
									<button onClick={() => removeCategory(category)} className="text-red-500 hover:text-red-700 text-xs">
										<Trash2 size={12} />
									</button>
								)}
							</div>
					...
					<div className="flex gap-2">
						...
						/>
						<button onClick={addCategory} className="bg-gray-500 text-white px-3 py-2 rounded hover:bg-gray-600">
							<Plus size={14} />
						</button>
					</div>
				</div>
				...
				<div className="mb-6">
					...
					<button onClick={addCustomClass} className="w-full bg-blue-500 text-white p-2 rounded hover:bg-blue-600 flex items-center justify-center gap-2">
						<Plus size={16} />
						Добавить класс
					</button>
				</div>
				...
				<div className="space-y-2 mb-4">
					<button onClick={exportCode} className="w-full bg-green-500 text-white p-2 rounded hover:bg-green-600 flex items-center justify-center gap-2">
						<Download size={16} />
						Экспорт кода
					</button>
					<button onClick={exportArchitectureJSON} className="w-full bg-blue-500 text-white p-2 rounded hover:bg-blue-600 flex items-center justify-center gap-2">
						<FileText size={16} />
						Экспорт JSON
					</button>
				</div>
					...
					<div className="mt-6 border-t pt-4">
						...
						<div className="mb-4">
							<div className="flex justify-between items-center mb-2">
								...
								<button onClick={() => addProperty(selectedClass.id)} className="text-blue-500 hover:text-blue-700">
									<Plus size={14} />
								</button>
							</div>
						...
						<div className="mb-4">
							<div className="flex justify-between items-center mb-2">
								...
								<button onClick={() => addMethod(selectedClass.id)} className="text-blue-500 hover:text-blue-700">
									<Plus size={14} />
								</button>
							</div>
							<div className="space-y-1 max-h-32 overflow-y-auto">
						...
						<button onClick={() => deleteClass(selectedClass.id)} className="w-full bg-red-500 text-white p-2 rounded hover:bg-red-600 flex items-center justify-center gap-2">
							<Trash2 size={16} />
							Удалить класс
						</button>
					</div>
				...
				>
					...
					>
						{classes.map((classObj) => (
							<div
								key={classObj.id}
								className={`class-block absolute bg-white border-2 rounded-lg shadow-md cursor-move min-w-48 ${getDynamicClassColor(classObj.type)} ${
									selectedClass?.id === classObj.id ? "ring-2 ring-blue-500" : ""
								}`}
								style={{
